      ...
      <div className="container-fluid px-0">
        <div className="container mb-4">
          <div className="d-flex justify-content-between align-items-end">
            ...
            <div>
              <Link to="/shop" className="view-all-link text-uppercase fw-bold text-decoration-none text-black fs-7 font-barlow">ALL BRANDS <span className="arrow-shift">&rarr;</span></Link>
            </div>
          </div>
        </div>
        ...
        <div className="brand-ticker-container position-relative overflow-hidden w-100 py-2">
          <div className="brand-ticker-track d-flex">
            {brands.concat(brands).map((b, i) => (
              <div className="brand-card-item" data-brand={b.key.toLowerCase().replace(/\s+/g, '-')} key={`${b.key}-${i}`}>
                <Link to={`/shop?brand=${encodeURIComponent(b.key)}&q=${encodeURIComponent(b.alt)}`} className="brand-card-inner d-flex align-items-center justify-content-center border-0 border-opacity-25 p-3 text-decoration-none">
